"""
Verify: Why Cooperative AI Outperforms Adversarial AI
======================================================
...
if __name__ == "__main__":
    ...
    # FIGURE DATA EXPORT
    # -------------------------------------------------------------------
    section("FIGURE DATA — Cooperative AI Equilibrium")

    from modules.figure_data import save_figure_data

    # Panel (a): Payoff matrix heatmap across friction regimes
    Phi_vals = np.array([1.0, 1.5, 2.2, 3.0, 4.0])
    payoff_matrix = np.zeros((len(Phi_vals), 4))
    for i, phi_v in enumerate(Phi_vals):
        T_v = compute_T(E_R, theta, delta_off, kappa)
        R_v = compute_R(E_R, c_coord)
        P_v = compute_P(E_R, phi_v)
        S_v = compute_S(theta, E_R, kappa, delta_def)
