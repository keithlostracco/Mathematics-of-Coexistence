...
\usepackage{amsmath}
\usepackage{amsthm}
% Indicator function: \mathbbm{1} renders as bold 1 (no external package needed)
...
\(g_{Bk}(\mathbf{x}_A) = \mathbbm{1}[\mathbf{x}_A \cap \Omega_B \neq \emptyset]\)
...
for conceptual clarity. Since \(\mathbbm{1}[\cdot]\) is neither
...
$$\pi_i = \frac{\alpha \sum_{j=1}^{N} c_j \cdot \mathbbm{1}[j \text{ cooperates}]}{N} - c_i \cdot \mathbbm{1}[i \text{ cooperates}] - \phi \cdot \bar{E} \cdot \frac{N - n_C}{N}$$
