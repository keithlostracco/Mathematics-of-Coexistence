...
where $\eta_{\text{info}} > 0$ is the \textbf{deception-friction sensitivity}, the proportionality constant mapping informational entropy (weighted by its energetic cost) to network friction. This generalizes the friction injection framework of \citep{TC-IV} with $v = \Delta H \cdot W_{\text{bit}}$ being the energetic equivalent of the injected information entropy.
\end{definition}
...
\citep{TC-IV} (\(M\) is the per-period transaction count, as above).
...
\(\nu\) denotes the per-period violation rate of \citep{TC-IV}):
...
\citep{TC-IV} \\
\(\nu\) & Per-period violation rate, as in \citep{TC-IV} \\
